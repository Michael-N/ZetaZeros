...
class PlottingUtil():
    ...
    @staticmethod
    def plot(points_tuple_list,x_name="x",y_name="y",title="Graph"):
        #setup
        plt.title(title)
        plt.xlabel(x_name)
        plt.ylabel(y_name)
        ...
        for points_tuple in points_tuple_list:
            #Plot the points
            plt.scatter(points_tuple[0], points_tuple[1], c=[(random.random(),random.random(),random.random())], s=np.pi * 3, alpha=0.5, label=str(idx))
            idx+=1
